...
(defcustom cperl-use-v6 nil
  "*Non-nil means that CPerl parses for Perl6 syntax."
  :type 'boolean
  :group 'cperl)

(defun cperl-looks-like-perl6 ()
...
;;;	  cperl-maybe-white-and-comment-rex	; 16=pre-block
  ...
  (make-local-variable 'cperl-use-v6)
  (if (cperl-looks-like-perl6)
      (set 'cperl-use-v6 t)
    (set 'cperl-use-v6 nil)
    )
...
;;;	       "\\=\\(\\\\.\\|[^][\\\\]\\|\\[:\\^?\sw+:]\\|\\[[^:]\\)*]")
		...
		(if (not cperl-use-v6)  ; perl6: for perl6 we just use ordinary highlighting in sub params
		    (progn
...
>>>>>>> origin/mxd-perl6-merge
	    ...
	    (list (concat "\\<" cperl-sub-regexp
		  ...
		  2 (if cperl-font-lock-multiline
            ...
            ;; nested parens in it.
	    '("\\<\\(package\\|class\\|module\\|namespace\\|role\\|grammar\\|rule\\|token\\|regex\\|require\\|use\\|import\\|no\\|bootstrap\\|\\(?:\\(?:multi\\|proto\\) \\)?method\\|before\\|after\\|around\\|override\\|augment\\)[ \t]+\\(?:#.+\n\\|[ \t]*\n\\)?[ \t]*\\([a-zA-z_][a-zA-z_0-9:]*\\)\\([ \t;]\\|$\\)" ; require A if B;
...
(defun cperl-make-regexp-x ()
  ...
  (save-excursion
    ...
    (if (looking-at "\\s|")
	nil				; good already
      (if (or (looking-at "\\([smy]\\|qr|rx\\)\\s|") ; perl6: rx
